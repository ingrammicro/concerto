...
func (dm *SaasAccountService) GetSaasAccountList() (saasAccounts []types.SaasAccount, err error) {
	...
	data, status, err := dm.concertoService.Get("/v2/settings/saas_accounts")
	if err != nil {
		return nil, err
	}
...
func (dm *SaasAccountService) CreateSaasAccount(saasAccountVector *map[string]interface{}) (saasAccount *types.SaasAccount, err error) {
	...
	data, status, err := dm.concertoService.Post("/v2/settings/saas_accounts/", saasAccountVector)
	if err != nil {
		return nil, err
	}
...
func (dm *SaasAccountService) UpdateSaasAccount(saasAccountVector *map[string]interface{}, ID string) (saasAccount *types.SaasAccount, err error) {
	...
	data, status, err := dm.concertoService.Put(fmt.Sprintf("/v2/settings/saas_accounts/%s", ID), saasAccountVector)
	if err != nil {
		return nil, err
	}
...
func (dm *SaasAccountService) DeleteSaasAccount(ID string) (err error) {
	...
	data, status, err := dm.concertoService.Delete(fmt.Sprintf("/v2/settings/saas_accounts/%s", ID))
	if err != nil {
		return err
	}
